...
int main(int argc, char *argv[]) {
  ...
  /* Start playing */
  ret = gst_element_set_state (data.pipeline, GST_STATE_PLAYING); //设置目标状态，gst_element_set_state() 会自动处理状态转换，包括缓冲区、缓冲区大小、缓冲区填充等。
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if ((audio_sink_pad && gst_pad_is_linked (audio_sink_pad)) && (video_sink_pad && gst_pad_is_linked (video_sink_pad))) {
    g_print ("We are already linked. Ignoring.\n");
    goto exit;
  }

  /* Check the new pad's type */
  new_pad_caps = gst_pad_get_current_caps (new_pad); //获取当前已经确定的cap(格式)
  if (new_pad_caps == NULL) {
    /* 某些情况下当前 caps 可能为 NULL，退回查询可能的 caps */
  ...
  if (g_str_has_prefix (new_pad_type, "audio/x-raw")) {
    if (audio_sink_pad == NULL) {
      g_printerr ("Audio sink pad not found on audioconvert.\n");
      goto exit;
    }
    ret = gst_pad_link (new_pad, audio_sink_pad);
  } else if (g_str_has_prefix (new_pad_type, "video/x-raw")) {
    if (video_sink_pad == NULL) {
      g_printerr ("Video sink pad not found on videoconvert.\n");
      goto exit;
    }
